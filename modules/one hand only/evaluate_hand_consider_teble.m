...
hands_w_specific_card = (cards_left-1)*2;
...
for value = fliplr(toak_value:14)
    cards_required_for_toak = 3 - table_value_hist(value);
    if cards_required_for_toak == 3
        %         p_has(3) = 1;
        break;
    elseif cards_required_for_toak == 2
        
        output.p_gets_beaten = output.p_gets_beaten + (1 - output.p_gets_beaten) * ...
            3*p_specific_hand(2);
    elseif cards_required_for_toak == 1
...
top_pair = 0;
pairs = card_value_hist(card_value_hist == 2);
if length(pairs) >= 2
    top_pair = pairs(end);
    %has 2 pair
    output.value = 2;
...
% pairs_on_tabe = tables_values_hist(tables_values_hist == 2);
...
if length(pair_on_tabe) == 1
    ...
    output.p_gets_beaten = output.p_gets_beaten + (1 - output.p_gets_beaten) * ...
        cards_on_table * p_specific_hand(1)...
        + (cards_on_table-1) * p_specific_hand(1);
end

pair_value = find(card_value_hist == 2);
if isempty(pair_value)
    pair_value = 0;
else
    pair_value = max(pair_value);
end

for value = fliplr(pair_value:14)
    cards_required_for_pair = 2 - table_value_hist(value);
    if cards_required_for_pair == 2
        %p_has(1) = 1;
        break;
    elseif cards_required_for_pair == 1
